/*--------------------------------------------------------------------------
// Description: hal_arm335xCAPPWM.c
// 
//
// Author(s): Josh Lane
...
/* Module information */
#define MODNAME "hal_arm335xCAPPWM"
MODULE_AUTHOR("Josh Lane");
MODULE_DESCRIPTION("PWMSS CAP PWM HAL driver for ARM335x");
MODULE_LICENSE("GPL");
...
RTAPI_MP_ARRAY_STRING(pwmgens, MAX_PWM, "name of eCAP modules");
int frequency = 0;
...
static int comp_id;
static ePWM_t *ePWM_array; /* pointer to array of eCAP_t structs in
                                    shmem, 1 per pwmgen */
static int howmany;
    ...
    for (n = 0; n < howmany; n++) {
        ...
        for(i=0; devices[i].name; i++) {
            ...
            if (retval == 0 ) {	//input pwmgen name matches ePWM0, ePWM1, or ePWM2
                ...
                ePWM->ePWM_reg = (void*) ePWM->pwmss_reg + 0x100;		//change 0x200 to ePWM_OFFSET
